...
data = pd.read_csv("4_diary_main.csv", encoding='latin-1',
                   usecols=['ID', 'BEGIN', 'Q401'],
                   converters={"Q401": get_activity, "BEGIN": convert_time}
                    )
data = data.sort_values(["ID", "BEGIN"])
data = data.rename(columns=({"ID": "sample_id", "BEGIN": "date_time", "Q401": "state"}))
data = data.iloc[:int(len(data)/4)]

d3 = D3Blocks()

d3.movingbubbles(data, datetime="date_time", sample_id="sample_id", state="state", filepath="movingpoints.html",
                 note="How VMese people spend their time", cmap="hsv", center="Travelling", figsize=(780, 800), size=2)